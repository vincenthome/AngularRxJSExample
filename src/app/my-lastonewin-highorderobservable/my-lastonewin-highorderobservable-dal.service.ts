...
import { catchError, map, switchMap, concatAll } from 'rxjs/operators';
import { UnknownResource, ReqResResponse, User } from '../models/user';

@Injectable({
  providedIn: 'root'
})
export class MyLastonewinHighorderobservableDalService {
  ...
  unknownResource$ = this.httpClient.get<ReqResResponse>(`${this.getUsersUrl}/?delay=3`)
    .pipe(
      ...
      concatAll(),
      switchMap(user => this.httpClient.get<ReqResResponse>(`${this.getUnknownResourceUrl}/${user.id}?delay=1`)),
